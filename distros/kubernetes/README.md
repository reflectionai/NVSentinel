...
GPU startup checks via a **mutating admission webhook** (DCGM diagnostics; optional NCCL tests). Disabled by default. Requires namespaces to be labeled for injection (for example `nvsentinel.nvidia.com/preflight=enabled`). Multi-node checks use **gang discovery** (native Workload API or PodGroup-style schedulers like Volcano and Run:ai) and either ConfigMap coordination or the workload's static torchrun environment. See the [Preflight configuration guide](../../docs/configuration/preflight.md) for details.
