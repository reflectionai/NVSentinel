...
func (m *MockReader) ReadIBPortCounter(device string, port int, counterPath string) (uint64, error) {
	if m.ReadIBPortCounterFunc != nil {
		return m.ReadIBPortCounterFunc(device, port, counterPath)
	}
