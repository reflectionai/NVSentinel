...
This module is the single source of truth for how preflight checks emit
logs. Each check should call `set_default_structured_logger` exactly once
at startup (typically via `nvsentinel_preflight_runtime.bootstrap()`).

The configured root logger emits one JSON object per line to stderr, with
every entry carrying `module`, `version`, `level`, and ISO `timestamp`
fields, plus any `extra={...}` keyword fields the caller supplies. Calls
to `structlog.contextvars.bind_contextvars(...)` add fields that are
included on every subsequent log line from the same process — this is how
runtime context (rank, pod_name, etc.) gets injected automatically.
"""
...
def set_default_structured_logger(
  module: str, version: str, level: str
) -> None:
