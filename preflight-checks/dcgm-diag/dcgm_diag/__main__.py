...
def main() -> None:
  bootstrap(module='preflight-dcgm-diag', version=get_version())
  ...
  log.info(
    ...
    extra={
      ...
      'processing_strategy': pb.ProcessingStrategy.Name(
        cfg.processing_strategy
      ),
    },
  )
...
def _run_diagnostic(
  cfg: Config, reporter: HealthReporter, diag: DCGMDiagnostic
) -> int:
  log = logging.getLogger(__name__)
  ...
  try:
    ...
    try:
      reporter.send_event(
        gpu_uuid='', is_healthy=False, is_fatal=True, message=str(e)
      )
  ...
  for r in results:
    ...
    log.log(
      logging.INFO
      if is_pass
      else (logging.ERROR if is_fatal else logging.WARNING),
      f'Test {r.status}',
      extra={
        'gpu': r.gpu_uuid,
        'test': r.test_name,
