...
"""NCCL Pairwise preflight check entry point.
...
The flow is:
    entrypoint.py → torchrun → nccl_pairwise.__main__
...
def get_version() -> str:
    """Get package version."""
    try:
        return version("nvsentinel-nccl-pairwise")
    except PackageNotFoundError:
        return "dev"


def main() -> None:
    """Main entry point for the NCCL pairwise benchmark.

    This is called by torchrun on each process. Only rank 0 sends
    health events and determines the final exit code.
    """
    log_level = os.getenv("LOG_LEVEL", "info")
    set_default_structured_logger("preflight-nccl-pairwise", get_version(), log_level)
...
def run() -> int:
    """Run the multi-node NCCL pairwise bandwidth benchmark.

    Initialises a NCCL process group (one process per GPU across all gang
    members), executes pairwise all-reduce operations at the configured message sizes,
...
def _run_benchmark(cfg: Config, rank: int) -> int:
    ...
    log.info(
        "Running NCCL pairwise benchmark",
        extra={
            "rank": rank,
            "world_size": dist.get_world_size(),
    ...
    try:
        ...
        if rank == 0:
            return _handle_failure(
                cfg,
                NCCLError.PAIRWISE_TIMEOUT,
                f"NCCL pairwise benchmark failed: {err}",
            )
        return NCCLError.PAIRWISE_TIMEOUT.value.exit_code

    passed = result.passed or cfg.skip_bandwidth_check

    # Only rank 0 reports results and determines exit code
    if rank != 0:
        return NCCLError.SUCCESS.value.exit_code if passed else NCCLError.PAIRWISE_BW_DEGRADED.value.exit_code

    if cfg.skip_bandwidth_check and not result.passed:
        log.info(
            "NCCL pairwise check PASSED (bandwidth check skipped)",
    ...
    return _handle_failure(
        ...
        f"NCCL pairwise bus bandwidth {result.min_bus_bw:.2f} GB/s is below threshold {cfg.bw_threshold_gbps:.2f} GB/s",
    )
...
def _handle_success(cfg: Config, result: BenchmarkResult) -> int:
    ...
    """
    message = (
        f"NCCL pairwise bus bandwidth {result.min_bus_bw:.2f} GB/s meets threshold {cfg.bw_threshold_gbps:.2f} GB/s"
    )

    log.info("NCCL pairwise check PASSED", extra={"details": message})
...
def _handle_failure(cfg: Config, error: NCCLError, message: str) -> int:
    ...
    Returns:
        Exit code.
    """
    log.error("NCCL pairwise check FAILED", extra={"details": message})
