...
"""NCCL Pairwise error definitions.
...
class NCCLError(Enum):
    """NCCL Pairwise errors with their definitions."""
    ...
    PAIRWISE_BW_DEGRADED = ErrorDef(
        exit_code=1,
        error_code="NCCL_PAIRWISE_BW_DEGRADED",
        is_fatal=True,
        recommended_action=pb.RecommendedAction.CONTACT_SUPPORT,
    )
    PAIRWISE_TIMEOUT = ErrorDef(
        exit_code=1,
        error_code="NCCL_PAIRWISE_TIMEOUT",
        is_fatal=True,
        recommended_action=pb.RecommendedAction.CONTACT_SUPPORT,
    )
    PAIRWISE_INIT_FAILED = ErrorDef(
        exit_code=1,
        error_code="NCCL_PAIRWISE_INIT_FAILED",
        is_fatal=True,
        recommended_action=pb.RecommendedAction.CONTACT_SUPPORT,
    )
