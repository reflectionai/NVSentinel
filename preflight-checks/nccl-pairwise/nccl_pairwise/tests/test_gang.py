...
from nccl_pairwise.gang import GangConfig, GangConfigReader, GangWaiter, PeerInfo
    ...
    def test_parse_four_field_format(self, tmp_path: Path) -> None:
        ...
        write_configmap(
            config_dir,
            {
                "expected_count": "2",
                "peers": (
                    "pod-0;10.0.0.1;0;preflight-dcgm-diag,preflight-nccl-pairwise\n"
                    "pod-1;10.0.0.2;1;preflight-dcgm-diag,preflight-nccl-pairwise"
                ),
            },
        )
        ...
        assert len(cfg.peers) == 2
        assert cfg.peers[0].check_names == "preflight-dcgm-diag,preflight-nccl-pairwise"
        assert cfg.peers[1].check_names == "preflight-dcgm-diag,preflight-nccl-pairwise"
...
class TestValidatePeers:
    ...
    def test_same_checks(self) -> None:
        peers = [
            PeerInfo("pod-0", "10.0.0.1", 0, "preflight-dcgm-diag,preflight-nccl-pairwise"),
            PeerInfo("pod-1", "10.0.0.2", 1, "preflight-dcgm-diag,preflight-nccl-pairwise"),
        ]
    ...
    def test_check_mismatch(self) -> None:
        peers = [
            PeerInfo("pod-0", "10.0.0.1", 0, "preflight-dcgm-diag,preflight-nccl-pairwise"),
            PeerInfo("pod-1", "10.0.0.2", 1, "preflight-nccl-pairwise"),
        ]
        cfg = self._make_config(peers)
